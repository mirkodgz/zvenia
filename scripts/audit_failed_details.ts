...
async function run() {
    const targets = fullTargetListRaw.split('\n').map(t => t.trim()).filter(t => t.length > 5);

    // 1. Identify PENDING items (from DB)
    console.log("Checking DB Status for 96 items...");
    const pendingItems: { title: string, csvUrl?: string }[] = [];

    // Prelim map of MetaPDFs from CSV
        ...
        fs.createReadStream(CSV_PATH_POSTS)
            ...
            .on('data', (row: any) => {
                ...
                if (row.meta_pdf && row.meta_pdf.startsWith('http')) {
                    csvMap.set(normalize(title), row.meta_pdf);
                }
            })
            .on('end', resolve);
    });
    ...
    for (const t of targets) {
        // Use normalized lookup in map
        const normKey = normalize(t);
        let metaPdf = csvMap.get(normKey);

        // Check DB
        const { data } = await supabase.from('posts').select('document_url').ilike('title', `%${t.substring(0, 20)}%`).maybeSingle();

        const isRestored = data?.document_url && data.document_url.includes('cloudinary');
        if (!isRestored) {
            // Need to audit this one
            pendingItems.push({ title: t, csvUrl: metaPdf });
        }
    }

    console.log(`Auditing ${pendingItems.length} items...`);
    console.log("\n--- AUDIT RESULTS ---");

    for (const item of pendingItems) {
        if (!item.csvUrl) {
            console.log(`[MISSING CSV META] ${item.title}`);
            continue;
        }

        try {
            const resp = await axios.head(item.csvUrl, { timeout: 10000 });
            const size = parseInt(resp.headers['content-length'] || '0');
            const sizeMB = (size / (1024 * 1024)).toFixed(2);

            if (size > 20 * 1024 * 1024) { // 20MB
                console.log(`[TOO LARGE] ${item.title} (${sizeMB} MB)`);
            } else {
                console.log(`[OK SIZE/BUT FAILED] ${item.title} (${sizeMB} MB) - Status: ${resp.status}`);
            }
        } catch (e: any) {
            if (e.response) {
                console.log(`[HTTP ERROR ${e.response.status}] ${item.title}`);
            } else {
                console.log(`[CONNECTION ERROR] ${item.title} (${e.message})`);
            }
        }
    }
}
